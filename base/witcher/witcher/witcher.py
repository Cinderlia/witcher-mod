...
#            dictionary_vars.append(b"%s'(&" % inputvar.encode("utf-8"))
    ...
    #mod
    def start_fuzzer(self, do_resume, target_path, method_map, dictionary_str, seeds, requests=None, result_storage_pathname=None):
        if requests is None:
            requests = []
        ...
        for fuzzer_dir in os.listdir(self.work_dir):
            ...
            for src in glob.glob(os.path.join(cdir, 'id:*')):
                if src+url_path in self.saved_seeds:
                    continue
                fid = len(glob.glob(f"{crash_file_dpath}id*"))
                dst = os.path.join(crash_file_dpath, f"id:{fid:06},{encoded_url_path},src:{os.path.basename(src)},crash")
            ...
            for trial_index in range(0, nbr_trials):
                ...
                for refuzz_index in range(0, nbr_refuzzes):
                    ...
                    for target in targets[target_start: target_end]:
                        ...
                        #mod
                        start_results = self.start_fuzzer(do_resume, target_url, method_map, dictionary_str, seeds, target["requests"], result_storage_pathname=result_storage_pathname)
                        ...
                        run_xss_flow(self.work_dir, output_dir_name="xss_queue")
