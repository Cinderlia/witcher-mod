...
def generate_symbolic_execution_prompt(
    ...
    *,
    ...
    if input_seq is not None:
        ...
        seq_display = "?"
    lines.append("You are a professional code analysis assistant. Your task is to help the Web Fuzzer discover SQL injection vulnerabilities.")
    lines.append("")
    lines.append(
        "Symbolically execute the query expression of the SQL statement at line "
        + seq_display
        + ", representing it using external input expressions to form constraints for symbolic execution. Then solve these constraint expressions. Please modify environment variables and inputs to provide an external input (environment variables, COOKIE, POST, GET, SESSION) that can break the syntactic structure of the SQL statement."
    )
    lines.append("Note: The goal is not to exploit the vulnerability, but to construct inputs that cause the database to produce an SQL syntax error, thereby triggering the fuzz tool's error detection mechanism.")
    lines.append("Prefer inducing syntax errors. Prefer short payloads.")
    try:
    ...
    lines.append("")
    lines.append("General engineering priors (e.g., database NOT NULL, INSERT failure conditions, protocol specifications) may be used to infer which modifications are 'highly likely' to affect sql statement outcomes in real-world systems. However, do not assume specific schemas, field lengths, or hidden code.")
