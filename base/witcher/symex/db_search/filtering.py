...
def build_filter_prompt(overall_goal: str, phase_goal: str, executions, state: DBSearchState) -> str:
    ...
    lines = []
    lines.append("你是数据库查询结果过滤器。")
    lines.append("你的任务是根据总目标和当前轮次目标，从本轮数据库原始返回里筛选真正相关的有效信息。")
    lines.append("你只处理当前这一轮的查询集合，不要总结历史信息。")
    lines.append("本轮可能包含多条 SQL，它们服务于同一个目标，请合并筛选，不要逐条孤立看待。")
    lines.append("你只需要保留有用的查询及其对应的有用结果；这里的结果包括成功返回的关键数据，也包括和目标相关的数据库报错。")
    lines.append("如果无法确定某条结果是否关键，优先保留。")
    ...
    lines.append("")
    lines.append("本轮 SQL 及数据库原始返回：")
    lines.append(json.dumps(round_items, ensure_ascii=False, indent=2))
    lines.append("")
    lines.append("请输出如下 JSON：")
    lines.append("{")
    lines.append('  "query_result_pairs": [')
    lines.append('    {"sql": "原始 SQL", "result": {"rows": [{"column": "value"}]}}')
    lines.append("  ]")
    lines.append("}")
    lines.append("如果某条结果或报错被保留，必须把对应的原始 SQL 一起放进 query_result_pairs；只保留真正有用的 query-result 对，最多保留 10 条。")
    return "\n".join(lines).rstrip() + "\n"
...
def build_memory_filter_prompt(overall_goal: str, phase_goal: str, payloads, state: DBSearchState) -> str:
    ...
    lines = []
    lines.append("你是数据库历史信息过滤器。")
    lines.append("你的任务是根据总目标，对前面几轮已经筛选过的数据库信息再次筛选，只保留真正有用的 query-result 对。")
    lines.append("如果某条结果被保留，必须保留对应的原始 SQL 语句。")
    lines.append("和目标相关的数据库报错、空结果、表不存在、列不存在、权限异常、查询无命中等信息都可以保留。")
    lines.append("只输出 JSON。")
    lines.append("")
    lines.append("总目标：")
    lines.append(str(overall_goal or "<EMPTY_OVERALL_GOAL>"))
