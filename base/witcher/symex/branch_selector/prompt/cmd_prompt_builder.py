...
Guiding principle: It is better to make a false positive than to miss a potentially command injection point.

Explicit rules:
1. [MUST SELECT] If the variable in the command execution statement may originate directly or indirectly from user input (GET, POST, COOKIE, HTTP headers, etc.)
2. [MUST SELECT] If the variable in the command execution statement may originate from environment variables (e.g., getenv).
3. [MUST SELECT] If the variable in the command execution statement may originate from SESSION file storage.
4. [MUST SELECT] If the variable in the command execution statement may originate from database query results.
5. [MUST SELECT] If the variable in the command execution statement may originate from user-uploaded files or read file paths controlled by the user.
...
15. [SHOULD SELECT] If the variable name or usage pattern suggests it may be user input (e.g., $input, $param, $data), select the command execution statement.
16. [SHOULD SELECT] For business data variables, it is generally assumed that they may originate from user input.

If the explicit rules do not cover all possible scenarios, please select any command execution statement that is likely to be influenced by external inputs.

Output Format:
Only output a JSON array containing the numbers of the selected command execution statements. If no command execution statement is selected, return an empty array.
Example: [123, 456, 789]
