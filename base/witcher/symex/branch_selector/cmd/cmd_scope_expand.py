...
def _cmd_arg_nodes_from_call(call_id: int, nodes: dict, children_of: Dict[int, List[int]]) -> List[int]:
    ...
    if t in ("AST_CALL", "AST_STATIC_CALL"):
        ...
        if arg_list_id is None:
            return []
        arg_nodes = _sorted_children(int(arg_list_id), nodes, children_of)
        if not arg_nodes:
            return []
        return [int(arg_nodes[0])]
    if t in ("AST_SHELL_EXEC", "AST_BACKTICK"):
        return [int(c) for c in _sorted_children(int(call_id), nodes, children_of)]
    return []


def _collect_cmd_taints(call_ids: Iterable[int], nodes: dict, children_of: Dict[int, List[int]], seq: int) -> List[dict]:
    out = []
    seen = set()
    for cid in call_ids or []:
        try:
            ...
            continue
        arg_roots = _cmd_arg_nodes_from_call(root_id, nodes, children_of)
...
def _expand_one_cmd_seq(
    *,
    ...
    taints = _collect_cmd_taints(call_ids, nodes, children_of, seq_i)
...
def iter_cmd_sections(
    *,
    ...
    for rec in trace_index_records or []:
        ...
        if min_seq is None:
            continue
        hits = find_cmd_calls_in_record(rec, nodes, children_of)
        if not hits:
            continue
    ...
    if logger is not None:
        logger.info("collect_cmd_seqs_done", records=seen_records, seqs=yielded)
