...
def _collect_xss_taints(call_ids: Iterable[int], nodes: dict, children_of: Dict[int, List[int]], seq: int) -> List[dict]:
    out = []
    seen = set()
...
def _expand_one_xss_seq(
    *,
    ...
    taints = _collect_xss_taints(call_ids, nodes, children_of, seq_i)
...
def iter_xss_sections(
    *,
    ...
    for rec in trace_index_records or []:
        ...
        if min_seq is None:
            continue
        hits = find_xss_output_calls_in_record(rec, nodes, children_of)
        if not hits:
            continue
    ...
    if logger is not None:
        logger.info("collect_xss_seqs_done", records=seen_records, seqs=yielded)
