...
//        console.log("Starting formdatafrompage");
    ...
    async do_login(page, options={}){
        ...
        console.log(`[DEBUG] Navigation completed, status: ${response.status()}`);
        ...
        try {
            if (loginData["usernameSelector"] || loginData["passwordSelector"]){
                ...
                if (loginData["usernameSelector"]) {
                    console.log(`[MYDEBUG] Filling username: ${loginData["usernameValue"]}`);
                    await page.click(loginData["usernameSelector"], { clickCount: 3 });
                    await page.keyboard.press('Backspace');
                    await page.type(loginData["usernameSelector"], loginData["usernameValue"], { delay: 50 });
                }

                console.log(`[MYDEBUG] Filling password`);
                await page.click(loginData["passwordSelector"], { clickCount: 3 });
                await page.keyboard.press('Backspace');
                await page.type(loginData["passwordSelector"], loginData["passwordValue"], { delay: 50 });
                ...
                try {
                    if (submitType === "submit") {
                        console.log(`[MYDEBUG] Using submit() method`);
                        await page.$eval(loginData["form_selector"], form => form.submit());
                        
                    } else if (submitType === "enter") {
                        console.log(`[MYDEBUG] Pressing Enter key`);
                        await page.keyboard.press('Enter');
                        
                    } else if (submitType === "click") {
                        console.log(`[MYDEBUG] Clicking submit button`);
                        await page.click(loginData["form_submit_selector"]);
                    }
                    
                    console.log(`[MYDEBUG] Waiting for navigation after submit...`);
                    await page.waitForNavigation({ 
                        timeout: 15000, 
                        waitUntil: ['networkidle0', 'load'] 
                    });
                    console.log(`[MYDEBUG] Navigation after login completed`);
                    
                } catch (navError) {
                    console.log(`[MYDEBUG] Navigation timeout, but continuing. Error: ${navError.message}`);
                }
            } else {
        ...
        console.log(`[MYDEBUG] === SESSION STATUS CHECK ===`);
        const currentUrl = await page.url();
        const pageContent = await page.content();

        const isOnIndexPage = currentUrl.includes('index.php');
        console.log(`[MYDEBUG] On index page: ${isOnIndexPage}`);

        const pageCookies = await page.cookies();
        const sessionCookie = pageCookies.find(c => c.name.includes('sid'));
        console.log(`[MYDEBUG] Session cookie: ${sessionCookie ? 'EXISTS' : 'MISSING'}`);

        const isLoggedIn = pageContent.includes('Last visit was') || 
                        pageContent.includes('Logout') ||
                        pageContent.includes('Private Messages');

        console.log(`[MYDEBUG] Logged in indicators: ${isLoggedIn}`);
        console.log(`[MYDEBUG] Page contains 'Last visit was': ${pageContent.includes('Last visit was')}`);
        console.log(`[MYDEBUG] Page contains 'Logout': ${pageContent.includes('Logout')}`);
        console.log(`[MYDEBUG] Page contains 'Private Messages': ${pageContent.includes('Private Messages')}`);

        if (isOnIndexPage && sessionCookie && !isLoggedIn) {
            console.log(`[CRITICAL] On index page with session but no login indicators - LOGIN FAILED`);
        } else if (isLoggedIn) {
        ...
        //     process.exit(38);
        // }
        console.log(`[MYDEBUG] Checking for positive login message: ${loginData["positiveLoginMessage"]}`);
        if (bodyResponse.indexOf(loginData["positiveLoginMessage"]) === -1){
            console.log(`[MYDEBUG] Positive login message not found: ${loginData["positiveLoginMessage"]}`);
            console.log(`[MYDEBUG] Page content snippet: ${bodyResponse.substring(0, 500)}...`);
            console.log("\n[WARNING] Login validation failed, but continuing anyway...");
            // process.exit(38);
        } else {
